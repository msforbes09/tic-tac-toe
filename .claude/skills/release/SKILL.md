...
8. After merge, bring `develop` level with `main` so the version bump lands there too:
   `git checkout develop && git merge --ff-only origin/main && git push`. Skipping this leaves
   `develop` on the old version and the next release PR conflicts on `package.json`.
9. The user tests on laptop and phone. Remove any merged worktrees and branches.
