...
to first-to-6 series that everyone else can watch. Finished games are saved
to localStorage history; rooms and series results live in Supabase.
...
- `supabase/schema.sql` — the two tables, policies, and `delete_room`; run once per Supabase project.
...
- Online: the challenger's device is the referee and holds the `SeriesState`; the challenged player and watchers send requests / `hello` and apply `state` snapshots. The challenger is `p1`; the challenged player is X in game 1 and first move alternates. First to 6 of 10, tie breaker if level, resign = loss, 30 s grace on a drop. Players save each finished game to local history with their own symbol; watchers save nothing. Online is never the remembered setup mode.
