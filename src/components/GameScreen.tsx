...
export function GameScreen({
  ...
  const isBotTurn =
    botSymbol !== null && state.status === "playing" && nextPlayer(state.board) === botSymbol
  // Confetti when you beat the bot. Two players sharing a phone get none.
  const youWon =
    settings.mode === "bot" &&
    state.status === "won" &&
    state.winner !== null &&
    seatOf(state, state.winner) === "p1"
  ...
  useEffect(() => {
    ...
    const id = setTimeout(
      ...
      thinkTime(state.board, rung, index),
    )
  ...
  const record = useCallback(
    (game: GameState) => {
      const outcome: Outcome = game.status === "draw" ? "draw" : (game.winner as Outcome)
      const now = Date.now()
      ...
      const result: GameResult =
        outcome === "draw" ? "draw" : seatOf(game, outcome) === "p1" ? "win" : "loss"
    ...
    [settings, storage, ladder, rung, gamesPlayed, onRecorded, onAchievement, tone],
  )

  useEffect(() => {
    if (settings.mode === "online" || state.status === "playing" || state.recorded || state.voided)
      return
  ...
  }, [state, settings.mode, ladder, record])

  // Once each side has moved, a bot game is played out or resigned (the reducer's rule): New game is
  // off and Back reads Resign, recording the loss before leaving.
  const resigned = ladder ? resign(state) : null
  ...
  const devSuffix =
    dev && ladder
      ? ` · ${rung}${streak > 0 ? ` · +${streak}` : streak < 0 ? ` · −${-streak}` : ""}`
      : ""
  const badge =
    settings.mode === "bot" ? `Bot · ${DIFFICULTY_LABEL[shownBand]}${devSuffix}` : "Two player"
